...
export default function Auth({ onAuth, t, onClose, pendingPlan, setPendingPlan }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      else if (e.code === "auth/too-many-requests") setError("Too many attempts. Please wait.");
  ...
  return (
    ...
    >
      <div style={{ position:"relative", background:"#0f1729", border:"1px solid rgba(255,255,255,0.1)", borderRadius:16, padding:32, width:"100%", maxWidth:400 }}>
        ...
        <input placeholder="Email Address" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()}
          style={{ width:"100%", padding:12, borderRadius:8, border:"1px solid rgba(255,255,255,0.1)", background:"rgba(255,255,255,0.05)", color:"#fff", marginBottom:12, boxSizing:"border-box", fontFamily:"'Inter',sans-serif", fontSize:"0.9rem", outline:"none" }}
        />

        <input placeholder="Password (min 6 characters)" type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()}
          style={{ width:"100%", padding:12, borderRadius:8, border:"1px solid rgba(255,255,255,0.1)", background:"rgba(255,255,255,0.05)", color:"#fff", marginBottom:12, boxSizing:"border-box", fontFamily:"'Inter',sans-serif", fontSize:"0.9rem", outline:"none" }}
        />
